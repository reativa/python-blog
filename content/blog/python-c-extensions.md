...
date: "2020-01-05"
description: ''
title: Python tudo sobre C Extensions
tags: string,intermediate
--- 
============

Occasionally, it is unavoidable for pythoneers to write a C extension.
...
Note that the C extension interface is specific to official CPython. It
is likely that extension modules do not work on other Python
implementations such as [PyPy](https://pypy.org/). Even if official
...
use [ctypes](https://docs.python.org/3/library/ctypes.html) module or
...
wrapped by `Py_BEGIN_ALLOW_THREADS` and `Py_END_ALLOW_THREADS` for
...
If threads are created from C/C++, those threads do not hold the GIL.
Without acquiring the GIL, the interpreter cannot access Python
functions safely. For example
...
Functions** between `PyGILState_Ensure` and `PyGILState_Release` in C
extension code.
